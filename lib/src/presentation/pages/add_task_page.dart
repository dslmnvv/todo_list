...
class AddTaskPage extends StatelessWidget {
  const AddTaskPage({Key? key, this.index, this.task,}) : super(key: key);
...
class DescriptionField extends StatelessWidget {
  const DescriptionField({Key? key, required this.onChanged, required this.initialValue}) : super(key: key);
...
class PriorityContainer extends StatefulWidget {
  const PriorityContainer({Key? key, required this.onSelect, required this.priority}) : super(key: key);
...
class DatePickerContainer extends StatefulWidget {
  const DatePickerContainer({Key? key, required this.onSelect, required this.onSwitch, this.initialDate}) : super(key: key);
...
class DeleteButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextButton(
        style: (onRemove != null) ? TextButton.styleFrom(
          padding: EdgeInsets.zero,
        ...
        child:  Row(
          ...
          children: [
            Icon(Icons.delete),
            SizedBox(width: 10),
            Text('Удалить', style:  StyleLibrary.font.button.copyWith(
              color: (onRemove == null)
